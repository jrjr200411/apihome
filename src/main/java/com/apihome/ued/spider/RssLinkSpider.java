...
import com.apihome.ued.task.impl.SearchRssTaskImpl;
...
public class RssLinkSpider 
{
    ...
    {
    	return new StringBuffer().append("http://www.soso.com/q?w=").append(keyWord)
    	.append("&lr=&sc=web&ch=w.p&num=10&gid=&cin=&site=&sf=0&sd=0&nf=0&pg=").append(num).toString();
    }

    private String soUrl(String keyWord, int num)
    {
    	return new StringBuffer().append("http://www.so.com/s?q=").append(keyWord)
    	.append("&pn=").append(num).toString();
    }
    
    private String baiduUrl(String keyWord, int num)
    {
    	return new StringBuffer().append("http://www.baidu.com/s?wd=").append(keyWord)
    	.append("&pn=").append((num-1)*10).append("&ie=utf-8").toString();
    }
    
    private String googleUrl(String keyWord, int num)
    {
    	return new StringBuffer().append("http://www.google.com.hk/search?q=").append(keyWord)
